...
function askForNumber() {
  ...
  while (numMeals < 1 || numMeals > 6 || isNaN(numMeals)) {
    if (numMeals < 1) {
      numMeals = input.question("There must be at least one meal. Enter a new number: ");
    } else if (numMeals > 6) {
      numMeals = input.question("There must be 6 or less meals. Enter a new number: "); 
    } else if (isNaN(numMeals)) {
      numMeals = input.question("Your input must be a number. Enter a number: "); 
    } else {
      break; 
    }  
  }
...
function generatePassword(string1, string2) {
  ...
  for (i = 0; i < string1.length && i < string2.length; i++) {
    code = code.concat(string1.slice(i, i + 1) + string2.slice(i, i + 1)); 
  }
  return code;
}
